...
import { injected, bscWalletConnect, maticWalletConnect, ftmWalletConnect, walletConnect } from "./connector";
...
export const MetaMaskProvider = ({ children }) => {
  ...
  const connect = async (providerType) => {
    ...
    try {
      ...
      } else if (providerType === "maticWalletConnect") {
        await activate(maticWalletConnect).then(() => {
          localStorage.setItem("providerType", "maticWalletConnect");
          localStorage.setItem("isConnected", true);
          setShouldDisable(false);
          handleWalletModal(false);
        });
      } else if (providerType === "ftmWalletConnect") {
        await activate(ftmWalletConnect).then(() => {
          localStorage.setItem("providerType", "ftmWalletConnect");
          localStorage.setItem("isConnected", true);
          setShouldDisable(false);
          handleWalletModal(false);
        });
